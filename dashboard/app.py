...
def connectDB():
    cred = credentials.Certificate(r"C:\Users\Loaner\Desktop\cfp\phlask-admin\dashboard\phlask.json")
    ...
    if request.method == 'GET':
        for tap in db:
            if tap["tapnum"] == tapnum:
                ...
                tp.append(tap)
        return render_template("addtap.html", tap = tp[0])
    if request.method == 'POST':
        try:
            # tapcount = prod.get_count(water_prod)
            access = str(request.form["access"])
...
@dashboard.route('/viewtap/<int:tapnum>',methods = ['GET'])
def viewtap(tapnum):
    tp=[]
    db=prod.get_db(water_prod) 
    if request.method == 'GET':
        for tap in db:
            if tap["tapnum"] == tapnum:
                prod.get_tap(water_prod,tapnum)
                tp.append(tap)
        return render_template("viewtap.html", tap = tp[0])
...
def example():
    try:
        ...
        return render_template("test.html", taps=taps)
    except:
